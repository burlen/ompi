...
int ompi_coll_tuned_read_rules_config_file (char *fname, ompi_coll_alg_rule_t** rules, int n_collectives)
{
    ...
    if (!fname) {
        COLL_TUNED_ERROR("Gave NULL as rule table configuration file for tuned collectives... ignoring!\n");
        return (-1);
    }

    if (!rules) {
        COLL_TUNED_ERROR("Gave NULL as rule table result ptr!... ignoring!\n");
        return (-2);
    }

    if (n_collectives<1) {
        COLL_TUNED_ERROR("Gave %d as max number of collectives in the rule table configuration file for tuned collectives!... ignoring!\n", n_collectives);
        return (-3);
    }

    fptr = fopen (fname, "r");
    if (!fptr) {
        COLL_TUNED_ERROR("Cannot read rules file [%s]\n", fname);
        goto on_file_error;
    }
    ...
    if (NULL == alg_rules) {
        COLL_TUNED_ERROR("Cannot allocate rules for file [%s]\n", fname);
        goto on_file_error;
    }
    ...
    if( (getnext(fptr, &NCOL) < 0) || (NCOL < 0) ) {
        COLL_TUNED_ERROR("Could not read number of collectives in configuration file around line %d\n", fileline);
        goto on_file_error;
    }
    if (NCOL>n_collectives) {
        COLL_TUNED_ERROR("Number of collectives in configuration file %ld is greater than number of MPI collectives possible %d ??? error around line %d\n", NCOL, n_collectives, fileline);
        goto on_file_error;
    }
    ...
    for (x=0;x<NCOL;x++) { /* for each collective */
        ...
        if( (getnext(fptr, &COLID) < 0) || (COLID < 0) ) {
            COLL_TUNED_ERROR("Could not read next Collective id in configuration file around line %d\n", fileline);
            goto on_file_error;
        }
        if (COLID>=n_collectives) {
            COLL_TUNED_ERROR("Collective id in configuration file %ld is greater than MPI collectives possible %d. Error around line %d\n", COLID, n_collectives, fileline);
            goto on_file_error;
        }

        if (alg_rules[COLID].alg_rule_id != COLID) {
            COLL_TUNED_ERROR("Internal error in handling collective ID %ld\n", COLID);
            goto on_file_error;
        }
        COLL_TUNED_VERBOSE(80,"Reading dynamic rule for collective ID %ld\n", COLID);
        ...
        if( (getnext (fptr, &NCOMSIZES) < 0) || (NCOMSIZES < 0) ) {
            COLL_TUNED_ERROR("Could not read count of communicators for collective ID %ld at around line %d\n", COLID, fileline);
            goto on_file_error;
        }
        COLL_TUNED_VERBOSE(80,"Read communicator count %ld for dynamic rule for collective ID %ld\n", NCOMSIZES, COLID);
        alg_p->n_com_sizes = NCOMSIZES;
        alg_p->com_rules = ompi_coll_tuned_mk_com_rules (NCOMSIZES, COLID);
        if (NULL == alg_p->com_rules) {
            COLL_TUNED_ERROR("Cannot allocate com rules for file [%s]\n", fname);
            goto on_file_error;
        }
        ...
        for (ncs=0;ncs<NCOMSIZES;ncs++) {	/* for each comm size */
            ...
            if( (getnext (fptr, &COMSIZE) < 0) || (COMSIZE < 0) ) {
                COLL_TUNED_ERROR("Could not read communicator size for collective ID %ld com rule %d at around line %d\n",
                                 COLID, ncs, fileline);
                goto on_file_error;
            }
            ...
            if( (getnext (fptr, &NMSGSIZES) < 0) || (NMSGSIZES < 0) ) {
                COLL_TUNED_ERROR("Could not read number of message sizes for collective ID %ld com rule %d at around line %d\n",
                                 COLID, ncs, fileline);
                goto on_file_error;
            }
            COLL_TUNED_VERBOSE(80,"Read message count %ld for dynamic rule for collective ID %ld and comm size %ld\n", NMSGSIZES, COLID, COMSIZE);
            ...
            if (NULL == com_p->msg_rules) {
                COLL_TUNED_ERROR("Cannot allocate msg rules for file [%s]\n", fname);
                goto on_file_error;
            }
            ...
            for (nms=0;nms<NMSGSIZES;nms++) {	/* for each msg size */
                ...
                if( (getnext (fptr, &MSGSIZE) < 0) || (MSGSIZE < 0) ) {
                    COLL_TUNED_ERROR("Could not read message size for collective ID %ld com rule %d msg rule %d at around line %d\n",
                ...
                if( (getnext (fptr, &ALG) < 0) || (ALG < 0) ) {
                    COLL_TUNED_ERROR("Could not read target algorithm method for collective ID %ld com rule %d msg rule %d at around line %d\n",
                                     COLID, ncs, nms, fileline);
                    goto on_file_error;
                }
                ...
                if( (getnext (fptr, &FANINOUT) < 0) || (FANINOUT < 0) ) {
                    COLL_TUNED_ERROR("Could not read fan in/out topo for collective ID %ld com rule %d msg rule %d at around line %d\n",
                                     COLID, ncs, nms, fileline);
                    goto on_file_error;
                }
                ...
                if( (getnext (fptr, &SEGSIZE) < 0) || (SEGSIZE < 0) ) {
                    COLL_TUNED_ERROR("Could not read target segment size for collective ID %ld com rule %d msg rule %d at around line %d\n",
                                     COLID, ncs, nms, fileline);
                    goto on_file_error;
                }
                ...
                if( (version > 1) && isnext_digit(fptr) ) {
                    if( (getnext (fptr, &MAXREQ) < 0) || (MAXREQ < 0) ) {
                        COLL_TUNED_ERROR("Could not read max requests for collective ID %ld com rule %d msg rule %d at around line %d\n",
                                         COLID, ncs, nms, fileline);
                        goto on_file_error;
                    }
                ...
                if (!nms && MSGSIZE) {
                    COLL_TUNED_ERROR("All algorithms must specify a rule for message size of zero upwards always first!\n"
        ...
        total_alg_count++;
        COLL_TUNED_VERBOSE(80,"Done reading dynamic rule for collective ID %ld\n", COLID);

    } /* per collective */

    fclose (fptr);

    COLL_TUNED_VERBOSE(60,"RuleFile:%s, Version:%u, Collectives:%d, CommunicatorSizes:%d, MessageSizes:%d, Lines:%d\n",
                       fname, version, total_alg_count, total_com_count, total_msg_count, fileline);
 ...
 on_file_error:
    ...
    COLL_TUNED_ERROR("read_rules_config_file: bad configure file [%s]. Read afar as line %d\n"
